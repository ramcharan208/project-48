...
function draw() {
  ...
  if(gameState === play) {
    ...
    text("again press F11 and relaod the page to play again !",canvas.width/2-400,canvas.height/2-200)
    textFont("algerian")
    fill("red")
    text("PROJECT BY R.RAM CHARAN",canvas.width/2-400,canvas.height/2-100);
  ...
  if(gameState === instruction) {
    ...
    textFont("algerian");
    textSize(30);
    text("PROJECT BY R.RAM CHARAN",canvas.width/2-350,canvas.height/2+150)
    textSize(40);
    textFont("bold");
    stroke("white");
    fill("white");
    text("(*note- play game on fullScreen mode only for best experience",canvas.width/2-500,canvas.height/2+200);
    text("(click on F11 to go to fullscreen)*)",canvas.width/2-500,canvas.height/2+250)
    stroke("yellow");
    fill("yellow");
    textSize(35);
    ...
    text("  Save the people and the ",canvas.width/2-300,canvas.height/2-130);
    fill("blue");
    textSize(35);
    text("Earth !",canvas.width/2+60,canvas.height/2-130);
    fill("yellow")
    text("  Press the                   to shoot.",canvas.width/2-300,canvas.height/2-90);
    stroke("white");
    fill("white");
    text("'spaceBar'",canvas.width/2-150,canvas.height/2-90)
    text("Use the                                                to move.",canvas.width/2-300,canvas.height/2-50);
    fill("green")
    text("RIGHT AND LEFT Arrows",canvas.width/2-180,canvas.height/2-50)
    fill("yellow")
    text("  Press     to start the game.",canvas.width/2-300,canvas.height/2-10);
    fill("brown")
    text("'s'",canvas.width/2-200,canvas.height/2-10)
    if(keyDown("s")) {
      gameState = play;
    } 
    if(keyDown("r")) {
